...
class TestExportE2E:
    ...
    def test_export_to_s3_returns_export_id(self, lam, excavate_result, runs_bucket):
        ...
        result = invoke(lam, "claws-export", {
            ...
            "destination": {
                "type": "s3",
                "bucket": runs_bucket,
                "key": f"e2e-exports/{excavate_result['run_id']}.json",
            },
    ...
    def test_export_id_format(self, lam, excavate_result, runs_bucket):
        ...
        result = invoke(lam, "claws-export", {
            ...
            "destination": {
                "type": "s3",
                "bucket": runs_bucket,
                "key": f"e2e-exports/{excavate_result['run_id']}-check.json",
            },
        })
        if "error" in result:
            pytest.skip(f"export failed: {result}")
    ...
    def test_export_includes_provenance(self, lam, excavate_result, runs_bucket):
        """export with include_provenance=True returns a provenance field."""
        result = invoke(lam, "claws-export", {
            "run_id": excavate_result["run_id"],
            "destination": {
                "type": "s3",
                "bucket": runs_bucket,
                "key": f"e2e-exports/{excavate_result['run_id']}-prov.json",
            },
            "include_provenance": True,
        })
        if "error" in result:
            pytest.skip(f"export failed: {result}")
        assert "provenance" in result or "export_id" in result, \
            f"Expected provenance or export_id: {result}"
